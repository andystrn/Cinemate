  ...
  return (
    <main className="text-2xl">
      <section className="flex justify-around flex-wrap py-5">
        ...
        <div className="m-w-2xl text-gray-700 text-lg dark:text-white">
          <h1 className="text-4hl font-bold my-3 text-center lg:text-left">{movie.title}</h1>
          <p className="my-4">
            {movie.overview}
          </p>
          {movie.genres ? 
          <p className="my-7 flex flex-wrap gap-2"> 
             {movie.genres.map((genre) => <span key={genre.id} className="mr-2 border border-gray-200 rounded dark:border-dray-600 p-2">{genre.name}</span>)}
             </p> :
            null }
